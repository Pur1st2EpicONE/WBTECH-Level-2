...
The helper function getRepeatCount reads consecutive digits from the rune slice starting at a given position and converts them to an integer, returning 1 if no digits are found (default repeat count).

If the input is empty, an empty string is returned.  
If the entire input is numeric — returns **ErrNoCharacters**.  
If a digit appears without a preceding character — returns **ErrDigitWithoutChar**.  
